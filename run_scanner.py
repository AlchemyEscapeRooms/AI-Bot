...
def run_scan(args):
    ...
    high_score = [r for r in results if r.composite_score >= 75]
    med_score = [r for r in results if 60 <= r.composite_score < 75]
    low_score = [r for r in results if r.composite_score < 60]

    print(f"High potential (75+):  {len(high_score)} stocks")
    print(f"Medium potential (60-75): {len(med_score)} stocks")
...
Examples:
    ...
    parser.add_argument(
        ...
        type=float,
        default=75.0,
        help='Minimum score for auto-add (default: 75)'
    )
    parser.add_argument(
        '--max-add',
        type=int,
